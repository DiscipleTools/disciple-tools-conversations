    ...
    if (pages > 1){
      if (this.offset === 0) {
        return html`
          ...
          <button class="button small" @click=${() => this.offset = this.offset + this.perPage}>${this._nextArrowSVG()}</button>`
      }
      if (this.offset === (pages-1) * this.perPage) {
        return html`
        <button class="button small" @click=${() => this.offset = this.offset - this.perPage}>${this._prevArrowSVG()}</button>
        ${this._paginationButtonsRender(pages)}`;
      }
